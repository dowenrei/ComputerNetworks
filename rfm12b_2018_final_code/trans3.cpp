...
void receive(uint8_t* net_packet,uint8_t length){
    ...
    uint8_t data_length=net->length;
    //GET DATA
    ...
    printByte(net->dest);
    put_str("\n\rSequence Number:");
    printByte(net->ctrl[1]>>2);
    put_str("\n\rFragment Number:");
    printByte(net->ctrl[1] & 0x03);   
    put_str("\n\rMessage Length (including newline):");
    printByte(net->length);        
    put_str("\n\rData: ");
    put_strHex(data,net->length);
    put_str("\n\rChecksum: ");
    printByte(*(net_packet+5+net->length));
    put_ch(' ');
    printByte(*(net_packet+5+net->length+1));
    put_str("\n\r");
    #endif
    ...
    if ( (net->ctrl[0] & (1<<4)) == 0x10){
        ...
        //put_str("Received Message:");
        //put_strHex(data,data_length);
